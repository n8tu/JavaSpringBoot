...
<body>
    ...
    <div class="container w-50">
        <h1 class="display-2 text-primary mt-4">PokeBook</h1>
        <table class="table mt-4">
            ...
            <tbody>
                <c:forEach var="poke" items="${pokes}">
                    <tr>
                        <td>
                            <a href="/expenses/<c:out value="${poke.id}"/>">
                                <c:out value="${poke.expense_name}"/>
                            </a>
                        </td>
                        <td><c:out value="${poke.vendor}"/></td>
                        <td><c:out value="${poke.amount}"/></td>
                        <td class="d-flex justify-content-evenly align-items-center">
                            <a href="/expenses/edit/<c:out value='${poke.id}'/>">Edit</a>
                            <form:form method="post" action="/expenses/delete/${poke.id}">
        ...
        <h1 class="display-5 text-primary mt-4">Track an expense:</h1>
        <form:form action="/expenses/create" method="POST" modelAttribute="poke">
            <p>
                <form:label path="expense_name">Expense Name:</form:label>
                <form:input class="form-control" path="expense_name"/>
            </p>
            <p>
